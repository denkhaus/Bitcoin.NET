...
namespace BitcoinNET.RPCClient
{
	public partial class BatchRPC
	{
		...
		public new uint GetMemoryPool()
		{
			uint id = NewID;
			requests.Add
				(new RPCRequest("getmemorypool", null, id));
			return id;
		}

		public new uint GetMemoryPool(string Data)
		{
			uint id = NewID;
			requests.Add
				(new RPCRequest("getmemorypool", new Object[] { Data }, id));
			return id;
		}
		...
		public new uint ImportPrivKey(string BitcoinPrivKey, string Label = "")
		{
			uint id = NewID;
			requests.Add
				(new RPCRequest("importprivkey", new Object[] { BitcoinPrivKey, Label }, id));
			return id;
		}
		...
		public new uint SendMany(string FromAccount, IEnumerable<string> FromAddresses, IDictionary<string, decimal> ToBitcoinAddresses, int MinConf = 1, string Comment = "")
		{
			IEnumerable<string> AccountAddresses = (new string[] { FromAccount }).Concat(FromAddresses);
			uint id = NewID;
			requests.Add
				(new RPCRequest("sendmany", new Object[] { AccountAddresses, ToBitcoinAddresses, MinConf, Comment }, id));
			return id;
		}
